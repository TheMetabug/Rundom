...
namespace
{
	...
	AnimatedSpriteGameObject* explosion = 0;
	AnimatedSpriteGameObject* snipe = 0;
	...
	void createNewExplosion()
	{
		...
		explosion = new AnimatedSpriteGameObject(0, spriteSheet);
		...
		explosion->addAnimation(0, SpriteAnimation::SpriteAnimationClip(indices,30.0f,1.0f, false));
		explosion->setActiveAnimation(0);
...
{	
	...
	for( int i=0; i<4; ++i )
	{
		...
		snipe->addAnimation(i, SpriteAnimation::SpriteAnimationClip(indices,animationFrameRate, 1.0f, true));
	}
	// Set animation at index 0 to active
	snipe->setActiveAnimation(0);
	// Set snipe position.
	...
	TextGameObject* text1 = createTextGameObject("Press ENTER to create new explosion" );
	text1->setPosition(0.5f*(text1->getText()->getWidth()/map->getTileWidth()),0);
	map->getLayer("GameObjects")->addGameObject(text1);

	TextGameObject* text2 = createTextGameObject("Press SPACE to change animation of Snipe-object" );
	text2->setPosition(0.5f*(text2->getText()->getWidth()/map->getTileWidth()),1);
...
void update( ESContext* ctx, float deltaTime )
{
	...
	if( (explosion != 0) && explosion->isFinished() )
	{
	...
	if(spaceKeyReleased)
	{
		int nextAnimation = (snipe->getActiveAnimation() + 1) % snipe->getNumAnimations();
		snipe->setActiveAnimation(nextAnimation);
	}
